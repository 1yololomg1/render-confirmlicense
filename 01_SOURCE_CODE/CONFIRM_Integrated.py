...
def check_offline_grace_period():
    """Check if we can use saved license when offline - allows indefinite use of saved license"""
    try:
        # Try to load saved license - if it exists, trust it (user entered it once, it should work forever)
        saved_license = get_saved_license()
        if saved_license and saved_license.get('license_key'):
            logger.info("Saved license found - using it for offline operation")
            # Extract metadata if available
            metadata = saved_license.get('metadata', {})
            # Return valid result with saved license info - user shouldn't have to re-enter
            return {
                "valid": True, 
                "tier": metadata.get('tier', 'unknown'), 
                "expires": metadata.get('expires', 'unknown'), 
                "reason": "Using saved license (offline mode)"
            }
        else:
...
def validate_license_activation():
    ...
    if saved_license_data:
        ...
        if validation_result["valid"]:
            ...
            # If it's a network/connection issue, use saved license anyway (user entered it once, trust it)
            # Only reject if license is actually invalid (revoked, expired, wrong machine, etc.)
            ...
            if is_network_error:
                logger.info("Network validation failed but saved license exists - using saved license")
                # Extract metadata from saved license
                metadata = saved_license_data.get('metadata', {})
                return {
                    "valid": True,
                    "tier": metadata.get('tier', 'unknown'),
                    "expires": metadata.get('expires', 'unknown'),
                    "reason": "Using saved license (network unavailable)"
                }
