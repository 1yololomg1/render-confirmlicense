...
Structure Overview:
- Configuration and persistence: load_or_create_config(), save_config(), get_saved_license(), save_license()
- License validation and enforcement: check_license_with_fingerprint(), check_offline_grace_period(),
  validate_license_activation()
- Security and fingerprinting: get_computer_fingerprint(), get_detailed_machine_info()
- UI workflows: LicenseDialog and validation handlers
- Infrastructure utilities: network retry helpers, threading helpers, startup checks
"""
...
def is_within_offline_grace_period(last_validated):
    ...
    try:
        ...
        logger.warning(f"Invalid last_validated timestamp format: {last_validated}")
        return False
...
def update_license_validation_state(validated_at: Optional[str] = None, offline_mode_enabled: bool = False):
    """
    Update license validation metadata in configuration.

    Args:
        validated_at: ISO timestamp of the most recent successful online validation.
        offline_mode_enabled: Whether the app is currently operating in offline mode.

    Side effects:
        Updates in-memory config and persists it to disk.
    """
    if not isinstance(app_config, dict):
        logger.warning("Configuration unavailable; cannot update license validation state")
        return

    license_info = app_config.setdefault("license_info", {})
    timestamp = validated_at or datetime.now().isoformat()

    license_info["last_validated"] = timestamp
    license_info["validation_count"] = (license_info.get("validation_count") or 0) + 1
    license_info["offline_mode_enabled"] = bool(offline_mode_enabled)

    if not save_config(app_config):
        logger.warning("Failed to persist license validation state")

def get_last_validated_timestamp(saved_license_data):
    """
    Resolve the most recent validation timestamp from saved license metadata or config.

    Args:
        saved_license_data: Dict returned by get_saved_license().

    Returns:
        ISO timestamp string, or None if not available.
    """
    if not isinstance(saved_license_data, dict):
        return None

    metadata = saved_license_data.get("metadata", {}) if isinstance(saved_license_data.get("metadata"), dict) else {}
    for key in ("validated_at", "last_validated"):
        value = metadata.get(key)
        if value:
            return value

    if isinstance(app_config, dict):
        license_info = app_config.get("license_info", {})
        if isinstance(license_info, dict):
            config_value = license_info.get("last_validated")
            if config_value:
                return config_value

    saved_at = saved_license_data.get("saved_at")
    return saved_at

def parse_expiry_timestamp(expires_value):
    """
    Parse an ISO expiry timestamp safely.

    Args:
        expires_value: Expiry value from license metadata.

    Returns:
        datetime instance if parseable, otherwise None.
    """
    if not expires_value:
        return None

    if not isinstance(expires_value, str):
        logger.warning("Expiry timestamp is not a string")
        return None

    normalized = expires_value.replace("Z", "+00:00")
    try:
        return datetime.fromisoformat(normalized)
    except ValueError:
        logger.warning(f"Invalid expiry timestamp format: {expires_value}")
        return None
    ...
    try:
        ...
        if hasattr(e, 'response') and e.response is not None:
            try:
                ...
                response_error = response_json.get('error') if isinstance(response_json, dict) else None
            except Exception:
                response_error = None
        if status_code == 503:
            ...
            user_message = response_error or "License revoked or inactive. Contact admin."
            logger.error(f"Access denied: {user_message}")
            return {"valid": False, "reason": user_message}
        else:
...
def check_offline_grace_period():
    """
    Check if offline use is permitted within the configured grace window.

    Returns:
        Dict with valid flag, tier, expires, and reason.
    """
    try:
        saved_license = get_saved_license()
        if not (saved_license and saved_license.get('license_key')):
            logger.warning("No saved license found for offline operation")
            return {"valid": False, "reason": "No saved license - internet connection required"}

        metadata = saved_license.get('metadata', {}) if isinstance(saved_license.get('metadata'), dict) else {}
        expires_value = metadata.get('expires')
        expiry_dt = parse_expiry_timestamp(expires_value)

        if expiry_dt and datetime.now(expiry_dt.tzinfo) > expiry_dt:
            logger.warning("Saved license is expired; offline use not allowed")
            return {"valid": False, "reason": "License expired - internet connection required"}

        last_validated = get_last_validated_timestamp(saved_license)
        if not is_within_offline_grace_period(last_validated):
            logger.warning("Offline grace period has expired; online validation required")
            return {"valid": False, "reason": "Offline grace period expired - internet connection required"}

        logger.info("Saved license is within offline grace period; allowing offline operation")
        update_license_validation_state(validated_at=last_validated, offline_mode_enabled=True)
        return {
            "valid": True,
            "tier": metadata.get('tier', 'unknown'),
            "expires": metadata.get('expires', 'unknown'),
            "reason": "Using saved license (offline grace period)"
        }
    except Exception as e:
        logger.error(f"Error checking saved license: {e}")
        return {"valid": False, "reason": "Error accessing saved license - internet connection required"}
    ...
    if saved_license_data:
        ...
        if validation_result["valid"]:
            ...
            try:
                ...
                update_license_validation_state(validated_at=validated_at, offline_mode_enabled=False)
                logger.debug("Updated license validation timestamp")
            except Exception as e:
                logger.warning(f"Failed to update license timestamp (non-critical): {e}")
            return validation_result
        else:
            logger.warning(f"Network validation failed: {validation_result.get('reason', 'Unknown error')}")
            reason = validation_result.get("reason", "")
            
            # If it's a network/connection issue, allow offline use only within grace period
            # Reject if license is invalid or grace period has expired
            network_errors = ["network", "timeout", "connection", "offline", "grace period"]
            is_network_error = any(err in reason.lower() for err in network_errors)
            
            if is_network_error:
                logger.info("Network validation failed; checking offline grace period")
                offline_result = check_offline_grace_period()
                if offline_result.get("valid"):
                    return offline_result
                logger.warning("Offline grace period unavailable; online validation required")
                return None
            
            # Only delete if license is actually invalid (not a network issue)
            logger.info("License is actually invalid (not just network issue) - removing saved license")
            try:
